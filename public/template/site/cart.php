                ...
                <div class="product-item-row">
                  ...
                  <div class="product-details-column">
                    ...
                    <input type="hidden" id="product_id" name="size_delete" value="<?php echo  $value['MaSize'] ?>">
                      ...
                      <p class="product-price" id="product-price_reduce"><?php if($row['GiaBanSauKM'] != ""){echo $row['GiaBanSauKM'].'đ'; }  ?> </p>
                      <div class="change_quantity">

                        <input type="number" min="0" max="<?php echo $value['SoLuongSize'] ?>" value="<?php echo $value['SoLuong'] ?>" id="quantity_product" name="quantity[<?= $value['MaSP'] ?>][<?= $value['MaSize'] ?>]" class="input_quantity" onchange="updateHiddenInput()">

                      </div>
                      <p class="product-subtotal" id="product-total">
                        <span class="total_product">Tổng tiền: </span>
                        
                        <?php if($row['GiaBanSauKM'] != ""){echo number_format( $row['GiaBanSauKM'] * $value['SoLuong'], 0, ",", ".");} else {
...
function updateHiddenInput() {
    ...
    var quantityValue = document.getElementById('quantity_product').value;
    
    // Thiết lập giá trị cho input hidden
    document.getElementById('quantity_hidden').value = quantityValue;
}
