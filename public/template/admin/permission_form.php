...
<form action="<?php echo $url.'/permission_controller' ?>" method="POST" id="permission_form">
    ...
    <div style=" padding: 20px;">
        ...
        <div class="input-group flex-nowrap" style="margin-top: 20px;">
            ...
            <input id="permission_id" name="permission_id" type="text" class="form-control" placeholder="Permission ID" aria-label="Username" aria-describedby="addon-wrapping" readonly>
        </div>
        <div class="input-group flex-nowrap" style="margin-top: 20px;border:0px">
            ...
            <input id="Permission_name" name="Permission_name" type="text" class="form-control" placeholder="Permission name" aria-label="Username" aria-describedby="addon-wrapping">
        </div>
        
        <input type="text" value="" id="memo" disabled  >

        <input id="btn-permission-form" type="submit" class="btn btn-success" style=" margin:0 auto;display:block; margin-top: 20px; " onclick="return inspect()" value=""></input>
    </div>
</form>
...
<script>
    function permission_form(maQuyen, tenQuyen, formName, action, buttonName) {
        ...
        document.getElementById("permission_id").value = maQuyen;
        document.getElementById("Permission_name").value = tenQuyen;
        document.getElementById("action").value = action;
        document.getElementById("btn-permission-form").value = buttonName;
        if (action == 'delete') {
            document.getElementById("Permission_name").readOnly = true;
        } else {
            document.getElementById("Permission_name").readOnly = false;
        }
